...
「決済APIを実装せよ」のような指示1つで、複数の家臣が並列に動く様子を示します。

### 1. 上様が指示を出す

```
上様: "Stripe決済APIを実装せよ。エンドポイント、テスト、ドキュメントも揃えること"
```

大名は指示を受け取ると、即座に参謀へ委譲します。上様はこの時点で次の指示を出せます。

### 2. 参謀がタスクを分解し、家臣に並列分配

参謀は指示を分析し、独立して実行可能な単位に分解します。

```
参謀: "3家臣に分配。ファイル競合なし、並列実行可能と判断。"
```

```text
家臣1 ← "POST /api/payments エンドポイントを実装せよ"
家臣2 ← "決済APIの単体テスト・統合テストを作成せよ"
家臣3 ← "決済APIのドキュメントを docs/api/payments.md に作成せよ"
          ↑ 3名が同時に作業開始
```
...
```text
家臣1: src/api/payments.ts, src/services/stripe.ts を作成 → 完了報告
家臣2: tests/api/payments.test.ts を作成              → 完了報告
家臣3: docs/api/payments.md を作成                    → 完了報告
       ↑ 全員の報告が参謀に集約される
```

### 4. 完了報告が上様に届く

参謀がダッシュボードを更新し、大名経由で上様に報告が届きます。

```
大名 → 上様: "Stripe決済API実装、全3タスク完了。
  - エンドポイント実装 (家臣1)
  - テスト作成 (家臣2)
  - ドキュメント作成 (家臣3)"
```

> 1人のエージェントなら順番に処理するところを、uesama では独立したタスクを家臣が並列に実行します。
> エージェント間の通信は YAML ファイル + tmux send-keys で行われ、ポーリングは発生しません。
