...
上様 → 大名: "Stripe決済APIを実装せよ。エンドポイント、テスト、ドキュメントも揃えること"
  ...
  参謀 → 家臣1: "POST /api/payments エンドポイントを実装せよ"
  参謀 → 家臣2: "決済APIの単体テスト・統合テストを作成せよ"
  参謀 → 家臣3: "決済APIのドキュメントを docs/api/payments.md に作成せよ"
                                        ── 3名が同時に作業開始 ──
...
> 上様が操作するのは最初の指示だけです。分解・分配・実行・集約・報告はすべてエージェント間で自律的に行われます。
