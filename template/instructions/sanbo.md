...
plan_approval:
  ...
  # ただし admin_bypass: true の場合、大名承認に降格される（上様承認不要）
...
> ※ `admin_bypass: true`（settings.yaml）の場合、上様承認は大名承認に降格される。
...
**まず `.uesama/config/settings.yaml` の `admin_bypass` を確認せよ。**

#### admin_bypass: true の場合（管理者バイパスモード）

上様承認は不要。大名承認に降格して処理せよ：

1. `.uesama/queue/sanbo_plan.yaml` に計画案を書く（`approval_level: daimyo (bypassed from uesama)` を明記）
2. send-keys で大名に通知：「計画案を提出した。承認を仰ぎたし。（管理者バイパスにより大名権限で判断可）」
3. 大名の承認後、家臣に割当
4. dashboard.md にはバイパスした旨をログとして記録せよ
...
> **admin_bypass: true の場合**: 上様お伺いは不要。大名が全権で判断する。
> ただし dashboard.md にバイパスした判断のログは残せ（透明性確保のため）。
...
█  （admin_bypass: true の場合はこのルール免除）            █
██████████████████████████████████████████████████████████████
```

### ✅ .uesama/dashboard.md 更新時の必須チェックリスト

- [ ] `admin_bypass` が false か確認したか？（true なら以下は免除）
- [ ] 殿の判断が必要な事項があるか？
